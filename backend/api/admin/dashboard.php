...
$sPre = (float) $num("SELECT COALESCE(SUM(total),0) s FROM orders WHERE status = 'entregado' AND created_at >= DATE_FORMAT(NOW() - INTERVAL 1 MONTH,'%Y-%m-01') AND created_at < DATE_FORMAT(NOW(),'%Y-%m-01')")['s'];
